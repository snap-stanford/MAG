...
with open('papers_by_prankh.tsv','w') as out_f:
    ...
    while it < PaperAA_PRIDJoinT.EndRI():
        ...
        if prev_id is not None:
            ...
            print >> out_f, '\t'.join([str(a) for a in [paper_title, paper_year, pr_score, indeg, first_auth, first_affl, second_auth, second_affl]])
...
with open('authors_by_prankh.tsv','w') as out_f:
    ...
    while it < PaperAA_PRIDJoinT.EndRI():
        ...
        if prev_id is not None:            
            print >> out_f, '\t'.join([str(a) for a in [auth_name, auth_aggr_score, auth_count, curr_min_year, curr_max_year]])
...
with open('affls_by_prankh.tsv','w') as out_f:
    ...
    while it < PaperAA_PRIDJoinT.EndRI():
        ...
        if prev_id is not None:
            print >> out_f, '\t'.join([str(a) for a in [affl_name, affl_aggr_score, affl_count, curr_min_year, curr_max_year]])
